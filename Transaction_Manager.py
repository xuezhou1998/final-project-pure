...
class Transaction_Manager:
    ...
    def update_sites_accessed(self, variable_id, trans_id, site_id):
    ...
    def get_site_variable_value(self, variable_id, site_id):
        return self.get_site(site_id).get_value(variable_id)

    def get_site_multi_version_variable_value(self, variable_id, site_id, trans_start_time):
    ...
    def get_last_write_commit_time(self, variable_id, site_id):
    ...
    def updated_site_variable_version(self, variable_id, site_id, version):
    ...
    def begin(self, trans_id):
        curr_transaction = Transaction(False, self.time_stamp)
        self.trans_map[trans_id] = curr_transaction
        return True

    def begin_read_only(self, trans_id):
        curr_transaction = Transaction(True, self.time_stamp)
        self.trans_map[trans_id] = curr_transaction
        return True

    def read(self, variable_id, trans_id):
        ...
        if curr_transaction.read_only:
            return self.read_read_only(variable_id, trans_id)
        ...
        if not self.is_replicated_variable(variable_id):
            ...
            if self.get_read_lock(variable_id, trans_id, site_id):
                self.update_sites_accessed(variable_id, trans_id, site_id)
                variable_value = self.get_site_variable_value(variable_id, site_id)
            ...
            for i in range(1, Constant.NUMBER_OF_SITES + 1):
                ...
                if self.is_replicated_variable(variable_id) and curr_site.just_recovery:
                    if self.get_last_write_commit_time(variable_id, i) < self.get_last_failure_time(i):
                        continue
                if self.get_read_lock(variable_id, trans_id, i):
                    self.update_sites_accessed(variable_id, trans_id, i)
                    variable_value = self.get_site_variable_value(variable_id, i)

                    print("Transaction T{} Read X{}: {}".format(trans_id, variable_id, variable_value))
                    return True
        return False

    def read_read_only(self, variable_id, trans_id):
        curr_transaction = self.get_transaction(trans_id)
        if not self.is_replicated_variable(variable_id):
            site_id = variable_id % Constant.NUMBER_OF_SITES + 1
            curr_site = self.get_site(site_id)
            if self.is_site_failed(site_id):
                ...
                variable_value = self.get_site_multi_version_variable_value(variable_id, site_id,
                                                                            curr_transaction.start_time)
            ...
            for k in range(1, Constant.NUMBER_OF_SITES + 1):
                ...
                last_write_commit_time = self.get_last_write_commit_time(variable_id, k)
                if (
                        not just_recovered) and curr_transaction.start_time > last_write_commit_time > self.get_last_failure_time(
                    k):
                    variable_value = self.get_site_multi_version_variable_value(variable_id, k,
                                                                                curr_transaction.start_time)
    ...
    def write(self, variable_id, trans_id, variable_value):
        ...
        if not self.is_replicated_variable(variable_id):
            ...
            if self.is_site_failed(site_id):
                return False
            curr_site = self.get_site(site_id)
            if curr_site.can_get_write_lock(variable_id, trans_id):
                curr_site.add_write_lock(variable_id, trans_id, self.time_stamp)
                curr_site.clear_wait_lock(variable_id, trans_id)
                ...
                curr_site.add_wait_lock(variable_id, trans_id, self.time_stamp)
            ...
            for i in range(1, Constant.NUMBER_OF_SITES + 1):
                if self.is_site_failed(i):
                    continue
                curr_site = self.get_site(i)
                if not curr_site.can_get_write_lock(variable_id, trans_id):
            ...
            if should_be_blocked:
                ...
                for id_item in id_list:
                    ...
                    self.update_sites_accessed(variable_id, trans_id, id_item)
                    curr_site.clear_wait_lock(variable_id, trans_id)
                curr_transaction.cache[variable_id] = variable_value
            ...
            for i in curr_transaction.sites_accessed_table.keys():
                ...
                if i in curr_transaction.cache.keys():
                    ...
                    for site_id_item in site_list:
                        ...
                        print("Transaction T{} Write X{}: {} on site {} at time {}".format(trans_id, variable_id,
                                                                                           variable_value, site_id_item,
                                                                                           self.time_stamp))
    ...
    def get_read_lock(self, variable_id, trans_id, site_id):
        if self.is_site_failed(site_id):
            return False
        curr_site = self.get_site(site_id)

        if curr_site.can_get_read_lock(variable_id, trans_id):
            curr_site.add_read_lock(variable_id, trans_id, self.time_stamp)
            return True
        else:
            curr_transaction = self.get_transaction(trans_id)
            wait_id = curr_site.get_waiting_id(variable_id, trans_id)
